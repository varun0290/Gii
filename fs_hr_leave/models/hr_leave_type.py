...
class HolidaysType(models.Model):
    ...
    allow_zero_allocation = fields.Boolean(string="Allow Zero Allocation")
        ...
        for employee in employees:
            for leave_type in leave_type_requires_allocation:
                ...
                for allocation, data in allocations_leaves_consumed[employee][leave_type].items():
                    ...
                    if allocation and leave_type.allow_zero_allocation and leave_type.hide_carry_forward_on_dashboard:
                        continue
                    if allocation:
                ...
                if not self.env.context.get('from_dashboard', False) or lt_info[1]['max_leaves'] or (leave_type.allow_zero_allocation and not leave_type.hide_carry_forward_on_dashboard and lt_info[1]['virtual_remaining_leaves'] == 0):
