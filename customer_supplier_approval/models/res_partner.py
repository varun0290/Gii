...
class ResPartner(models.Model):
    ...
    customer_supplier_id = fields.Char(
        string="ID",
        domain="[('state', '=', 'validated')]",
        help="Unique id for partners",
    )
    hide_button = fields.Boolean(
        default=False, help="This boolean field is used to hide the approve button"
    )
    hide_button_validate = fields.Boolean(
        default=False, help="This boolean field is used to hide the Validate button"
    )
    state = fields.Selection(
        ...
        default="draft",
        help="The states are in Draft,Validated and Approved",
    )
    _sql_constraints = [
