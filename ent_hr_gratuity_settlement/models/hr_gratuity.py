...
class EmployeeGratuity(models.Model):
    ...
    employee_joining_date = fields.Date(
        string="Joining Date", readonly=True, store=True, help="Employee joining date"
    )
    wage_type = fields.Selection(
        [("monthly", "Monthly Fixed Wage"), ("hourly", "Hourly Wage")],
        help="Select the wage type monthly or hourly",
    )
    total_working_years = fields.Float(
        string="Total Years Worked",
        readonly=True,
        store=True,
        help="Total working years",
    )
    employee_probation_years = fields.Float(
        string="Leaves Taken(Years)",
        readonly=True,
        store=True,
        help="Employee probation years",
    )
    employee_gratuity_years = fields.Float(
        string="Gratuity Calculation Years",
        readonly=True,
        store=True,
    ...
    employee_gratuity_duration = fields.Many2one(
        "gratuity.configuration", readonly=True, string="Configuration Line"
    )
    ...
    employee_gratuity_amount = fields.Float(
        string="Gratuity Payment",
        readonly=True,
        store=True,
    ...
    hr_gratuity_credit_account = fields.Many2one(
        "account.account", help="Gratuity credit account"
    )
    hr_gratuity_debit_account = fields.Many2one(
        "account.account", help="Gratuity debit account"
    )
    ...
    def _onchange_employee_id(self):
        ...
        if self.employee_id.id:
            ...
            conf_ids = (
                hr_accounting_configuration_id.gratuity_configuration_table.mapped("id")
            )
            hr_duration_config_ids = self.env["gratuity.configuration"].browse(conf_ids)
            for duration in hr_duration_config_ids:
                if (
                    duration.from_year
                    and duration.to_year
                    and duration.from_year
                    ...
                    duration.from_year
                    and not duration.to_year
                    and duration.from_year <= self.total_working_years
                ):
                    gratuity_duration_id = duration
                    break
                elif (
                    duration.to_year
                    and not duration.from_year
                    and self.total_working_years <= duration.to_year
                ):
                    gratuity_duration_id = duration
                    break
            ...
            #     )
            self.hr_gratuity_journal = (
                hr_accounting_configuration_id.gratuity_journal.id
            )
            self.hr_gratuity_credit_account = (
                hr_accounting_configuration_id.gratuity_credit_account.id
            )
            self.hr_gratuity_debit_account = (
                hr_accounting_configuration_id.gratuity_debit_account.id
            )
            ...
            if self.employee_gratuity_duration and self.wage_type == "hourly":
                if self.employee_gratuity_duration.employee_working_days != 0:
                    ...
                    raise Warning(
                        _(
                            "Employee working days is not configured in "
                ...
                if self.employee_gratuity_duration.employee_daily_wage_days != 0:
                    ...
                    employee_gratuity_years = 0.0
                    employee_gratuity_amt = 0
                    emp_year = 0
                    ...
                    if len(hr_duration_config_ids) == 1:
                        ...
                        employee_gratuity_years = round(employee_gratuity_years, 2)
                        emp_year += employee_gratuity_years
                        daily_wage = (
                            self.employee_basic_salary
                            / hr_duration_config_ids.employee_daily_wage_days
                        )
                        working_days_salary = daily_wage * employee_working_days
                        gratuity_pay_per_year = (
                            working_days_salary * hr_duration_config_ids.percentage
                        )
                        employee_gratuity_amount += (
                            gratuity_pay_per_year * employee_gratuity_years
                        )
                    else:
                        for duration in hr_duration_config_ids:
                            if (
                                duration.from_year <= self.employee_gratuity_years
                                and self.employee_gratuity_years <= duration.to_year
                            ):
                                if (
                                    duration.from_year
                                ...
                                employee_gratuity_years = round(
                                    employee_gratuity_years, 2
                                )
                                emp_year += employee_gratuity_years
                                daily_wage = (
                                    self.employee_basic_salary
                                    / duration.employee_daily_wage_days
                                )
                                working_days_salary = daily_wage * employee_working_days
                                gratuity_pay_per_year = (
                                    working_days_salary * duration.percentage
                                )
                                employee_gratuity_amount += (
                                    gratuity_pay_per_year * employee_gratuity_years
                                )
                                ...
                                duration.from_year < self.employee_gratuity_years
                                and duration.to_year < self.employee_gratuity_years
                            ):
                                if duration.to_year:
                                    employee_gratuity_years = round(
                                        abs(
                        ...
                        # employee_gratuity_amt = round(employee_gratuity_amount, 2)
                    self.employee_gratuity_amount = round(employee_gratuity_amount, 2)
                else:
                    raise Warning(
                        _(
                            "Employee wage days is not configured in "
                            "the gratuity configuration..!"
                        )
                    )
    ...
    def approved_request(self):
        for hr_gratuity_id in self:
            debit_vals = {
                ...
                "date": date.today(),
                "debit": hr_gratuity_id.employee_gratuity_amount > 0.0
                and hr_gratuity_id.employee_gratuity_amount
                or 0.0,
                "credit": hr_gratuity_id.employee_gratuity_amount < 0.0
                and -hr_gratuity_id.employee_gratuity_amount
                or 0.0,
            }
            credit_vals = {
